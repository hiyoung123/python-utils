...
def check_proxy(host, port, type):
    """
    检验代理是否可用
    """
    if 'HTTP' == type:
        proxies = {"http": "http://{host}:{port}".format(host=host, port=port)}
    elif 'HTTPS' == type:
        proxies = {"https": "https://{host}:{port}".format(host=host, port=port)}
    else:
        proxies = {
            "https": "https://{host}:{port}".format(host=host, port=port),
